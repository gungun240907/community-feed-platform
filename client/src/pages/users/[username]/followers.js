...
export default function FollowersPage() {
  ...
  useEffect(() => {
    ...
    userAPI
      ...
      .catch((err) => setError(err.response?.data?.error || 'Failed to load followers'))
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-surface-900">Followers</h1>
          <p className="text-sm text-surface-500">
            People following @{username}
            {!isLoading && <span className="ml-1">({followers.length})</span>}
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="card p-8 sm:p-16 text-center space-y-4">
          ...
          <p className="text-sm text-surface-400">
            When someone follows @{username}, they will appear here.
          </p>
        </div>
      ) : (
